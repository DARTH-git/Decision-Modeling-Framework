...
author: "Supplementary Material to: A need for change! A coding framework for improving transparency in decision modeling"
output: 
...
### 05: Analysis
The analysis component is where the elements in components 1-4 are combined to answer the question(s) of interest given current information and to quantify the value of potential further research. Our framework separates the analysis in three subcomponents, subcomponents: _05a Deterministic analysis_, _05b Uncertainty analysis_ and _05c Value of information analysis_. For the Sick-Sicker case-study, we use all three subcomponents to conduct the CEA and to quantify the uncertainty of our decision. For procedures in the cost-effectiveness analyses, we rely on the `R` package `dampack`, which is available here: 
...
This matrix is the result of the function. This compact matrix with the CE results can be used as an argument in the `calculate_icers` function from the `dampack` package to calculate the incremental cost-effectiveness ratios (ICERs) and noting which strategies are weakly and strongly dominated. 
...
In this subsection, we evaluate decision uncertainty by propagating the uncertainty through the CEA using probabilistic sensitivity analysis (PSA). Until now we used the parameter values as described in Table \ref{tab:parameters}. However, we are uncertain about these values. Most of these input parameters are defined by probability distribution as described in Table \ref{tab:parameters PSA}. 
...
The function returns a dataframe with a PSA dataset of the input parameters, that will be used to produce distributions of costs, effectiveness and NMB.
